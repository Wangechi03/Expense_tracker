...
app.config['MYSQL_PASSWORD'] = '@Wangeci03'
...
def register():
    ...
    if request.method == 'POST' :
        ...
        if account:
            ...
            return redirect(url_for('login'))  # Redirect to login or any other page after successful registration
...
def display():
    ...
    cursor = mysql.connection.cursor()
    cursor.execute('SELECT * FROM expenses WHERE user_id = % s AND date ORDER BY `expenses`.`date` DESC',(str(session['id'])))
...
def today():
      cursor = mysql.connection.cursor()
      cursor.execute('SELECT TIME(date)   , amount FROM expenses  WHERE user_id = %s AND DATE(date) = DATE(NOW()) ',(str(session['id'])))
